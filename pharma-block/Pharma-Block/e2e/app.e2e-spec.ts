...
describe('Starting tests for Pharma-Block', function() {
  ...
  it('website title should be Pharma-Block', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('Pharma-Block');
    })
  });

  it('network-name should be pharma-block@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('pharma-block@0.0.1.bna');
    });
  });

  it('navbar-brand should be Pharma-Block',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('Pharma-Block');
    });
  });
    ...
    it('Product table should have 18 columns',() => {
      page.navigateTo('/Product');
    ...
    it('FreightandTransport component should be loadable',() => {
      page.navigateTo('/FreightandTransport');
      browser.findElement(by.id('participantName'))
      .then((participantName) => {
        return participantName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('FreightandTransport');
      });
    });

    it('FreightandTransport table should have 5 columns',() => {
      page.navigateTo('/FreightandTransport');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(5); // Addition of 1 for 'Action' column
      });
    ...
    it('Customs component should be loadable',() => {
      page.navigateTo('/Customs');
      browser.findElement(by.id('participantName'))
      .then((participantName) => {
        return participantName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('Customs');
      });
    });

    it('Customs table should have 5 columns',() => {
      page.navigateTo('/Customs');
      element.all(by.css('.thead-cols th')).then(function(arr) {
        expect(arr.length).toEqual(5); // Addition of 1 for 'Action' column
      });
